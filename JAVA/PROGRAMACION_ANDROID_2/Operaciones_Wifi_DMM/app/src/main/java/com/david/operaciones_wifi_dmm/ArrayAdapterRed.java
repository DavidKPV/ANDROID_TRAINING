...
public class ArrayAdapterRed extends ArrayAdapter<Red> {
    ...
    public ArrayAdapterRed(Context contexto, int layoutResource, ArrayList<Red> lista) {
        ...
        this.contexto = contexto; this.layoutResource =layoutResource; this.lista = lista;
    }
    public View getView(int position, View convertView, ViewGroup parent){
        View row = convertView; HolderRed holder = null;
        if(row == null){
            LayoutInflater inflater = ((Activity)contexto).getLayoutInflater();
            row = inflater.inflate(layoutResource, parent, false);
            holder = new HolderRed();
            holder.ssid = (TextView) row.findViewById(R.id.tvSsid);
            holder.bssid = (TextView) row.findViewById(R.id.tvBssid);
            row.setTag(holder); }
        else{
    ...
    static class HolderRed{ TextView ssid; TextView bssid;}
}
